#!/usr/bin/env python3
"""Convert DP-700 practice-question + mock-exam markdown into adaptive-practice JSON banks.

Walks `certification/resources/practice-questions/*.md` for the practice bank,
and `certification/resources/mock-exam/questions.md`,
`certification/resources/mock-exam-2/questions.md`, and
`certification/resources/mock-exam-3/questions.md` for the three mock banks,
and emits up to four JSON files under `practice/data/` for the static practice
frontend to load.
...
Usage:
    ...
    python3 practice/build.py --kind mock           # all mock banks only
...
CERT_ID = "dp-700"
CERT_TITLE = "DP-700: Implementing Data Engineering Solutions Using Microsoft Fabric"
BLUEPRINT_VERSION = "2026-07-21"

# Number of mock exams to build (mock-exam/, mock-exam-2/, mock-exam-3/).
# The dp-700 guide ships 3 mocks (vs. dp-800's 2) — see certification/resources/
# mock-exam{,-2,-3}/questions.md. Directories may not exist yet (Tasks 7-9);
# build_mock() tolerates a missing questions.md by skipping that bank with a
# warning, so running build.py before those tasks land is safe and non-fatal.
MOCK_EXAM_COUNT = 3
...
# DP-700 uses `A.` choices (the official CLAUDE.md convention) but tolerate `A)` too.
CHOICE_RE = re.compile(r"^([A-D])[.)]\s*(.+?)\s*$")
ANSWER_CALLOUT_RE = re.compile(r"^>\s*\[!success\]-?\s*(?:Answer)?\s*$", re.IGNORECASE)
# Correct-answer markers seen in DP-700 mock + practice files:
#   **B. Always Encrypted ...**                    (canonical DP-700 form)
...
# Case-study H2 in mock exams: `## Case Study: Contoso Retail Migration *(5 linked questions, ~10 minutes)*`
...
def build_mock(exam_n: int, check_only: bool = False) -> dict:
    """Build a mock-exam bank for exam number `exam_n` (1, 2, or 3)."""
    ...
    if not md_path.exists():
        # Mock exam directories land in later tasks (Tasks 7-9) — a missing
        # questions.md here is expected until then, not an error.
        print(f"  No mock-exam file at {md_path} (not yet authored — skipping)", file=sys.stderr)
    ...
    # Pre-scan for domain boundaries — DP-700 mocks use HTML comments
...
def main() -> int:
    ...
    if args.kind in ("mock", "all"):
        print("\nMock exam banks:")
        for exam_n in range(1, MOCK_EXAM_COUNT + 1):
