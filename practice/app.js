...
(() => {
  ...
  const KNOWN_BANKS = [
    ...
    ...CERTS.map(c => ({ cert: `${c}-mock-3`, file: `data/${c}-mock-3.json`, group: "mock" })),
  ];
  const GROUP_LABELS = {
  ...
  const STORAGE_PREFIX = "dp700-practice-";
  const SESSION_HISTORY_PREFIX = "dp700-practice-sessions-";   // per-bank completed-session log
  const SESSION_HISTORY_CAP = 50;                              // ring-buffer cap per bank
  const THEME_KEY = "dp700-practice-theme";
  const TIMER_KEY = "dp700-practice-timer-minutes";
  const PASS_THRESHOLD = 70;                                   // % to pass DP-700 (700/1000 scaled)
  ...
  function renderBankPicker(certKey, items) {
    ...
    // Sort items: practice first, then mock-1, mock-2, mock-3, ... in order
    const sorted = [...items].sort((a, b) => {
      const order = it => {
        if (it.data.kind !== "mock") return 0;
        const m = it.data.cert.match(/-mock-(\d+)$/);
  ...
  function exportProgress() {
    ...
    a.href = url;
    a.download = `dp700-practice-${STATE.bank.cert}-progress.json`;
    a.click();
  ...
  // `dp700-practice-sessions-<cert>` (cap = SESSION_HISTORY_CAP).
...
<body><main>
  ...
  function init() {
    ...
    // Masthead links:
    //   DP-700 Practice brand → step 1 (all certifications)
