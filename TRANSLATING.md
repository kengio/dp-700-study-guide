# Translating the DP-700 Study Guide
...
└── <locale>/                        # BCP-47 locale tag (e.g., th, es, es-MX, ja, vi)
    ...
    └── certification/
        ├── dp-700-overview.md       # cert index (highest priority)
        ├── 01-fabric-workspace-settings/
        │   ├── fabric-workspace-settings.md
        │   └── 01-spark-settings.md
        ├── ...
        └── resources/
...
1. `certification/dp-700-overview.md` — the cert index
...
4. `certification/resources/cheat-sheets/` — all quick-references
5. Section landing pages (`01-fabric-workspace-settings/fabric-workspace-settings.md`, etc.)
6. Highest-churn topic files — start with orchestration and streaming (`04-orchestration/`, `08-streaming-data/`) since Fabric ships Real-Time Intelligence features fastest
7. Mock exams (`certification/resources/mock-exam*/questions.md`)
8. Everything else

## Conventions for translated files

- **Keep the file path identical** to the English source. `i18n/th/certification/04-orchestration/01-choosing-orchestration-tool.md` mirrors `certification/04-orchestration/01-choosing-orchestration-tool.md`.
- **Translate prose but keep code blocks untranslated.** PySpark, T-SQL, and KQL keywords, function names, error messages, JSON field names — leave in English. If a code comment is in English, you may translate the comment but leave the code itself unchanged.
...
- **Internal links should point within the locale tree.** `[Eventstreams](./02-eventstreams.md)` in a translated file should resolve to the translated sibling. If the sibling isn't translated yet, link directly to the English original: `[Eventstreams](../../../certification/08-streaming-data/02-eventstreams.md)` — and add a note (e.g., `*(English only)*`) so the reader isn't surprised.
...
- **Accuracy beats fluency.** If a Microsoft technical term has no good translation in your language, leave it in English (e.g., "Lakehouse", "OneLake", "Eventhouse", "shortcut") rather than coining an obscure native term.
