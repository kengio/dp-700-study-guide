# Contributing to the DP-700 Study Guide
...
- **The official Microsoft skills-measured page is the source of truth.** If a claim in this guide conflicts with the current [DP-700 study guide](https://learn.microsoft.com/en-us/credentials/certifications/resources/study-guides/dp-700), Microsoft wins. Always link the source for factual changes.
...
- Update the **"What's New"** callout at the top of `certification/dp-700-overview.md`
- Update the **"Updates"** section in `certification/resources/final-review.md`
...
- Move features from "Preview" to "GA" labelling as their status changes — verify against the [Microsoft Fabric documentation](https://learn.microsoft.com/en-us/fabric/) and the [Microsoft Fabric Blog](https://www.microsoft.com/microsoft-fabric/blog/)
...
- Code examples are `.md` files under `certification/resources/code-examples/{pyspark,tsql,kql}/` — never `.py`, `.sql`, or `.kql`
- Topic index files are named after the folder (e.g., `orchestration.md`), not `README.md`
...
- Language tags on all code blocks (`python`, `sql`, `kql`, `json`, `yaml`, `bash`)
...
If something is unclear about contributing, [open an issue](https://github.com/kengio/dp-700-study-guide/issues/new) and we'll iterate on this guide.
